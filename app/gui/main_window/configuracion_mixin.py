...
# ---------------- Configuración ----------------
    ...
    def tab_configuracion(self):
        ...
        for fam in ["Segoe UI", "Arial", "Tahoma"]:
            self.cfg_cmb_font.addItem(fam, fam)
        self.cfg_cmb_font.setCurrentText(th.get("font_family", "Segoe UI"))
...
# ===== PÁGINA: ACCESOS RÁPIDOS =====
    ...
    def _apply_config_from_ui(self):
        ...
        th["font_family"] = (self.cfg_cmb_font.currentData() or self.cfg_cmb_font.currentText() or "Segoe UI").strip()
        try:
        ...
        try:
            ...
            print("[config] no pude rearmar scheduler:", e)
        ...
        try:
            ...
            print("[config] error al reprogramar backups:", e)
    ...
    def _wire_reportes_guardar_programacion(self, page):
        ...
        def _reschedule():
            ...
            try:
                print("[Config] Guardar programación: rearmando scheduler de reportes…")
            except Exception:
                pass
...
# Capturar el widget interno y actualizar (el slot está en QPrintPreviewWidget)
