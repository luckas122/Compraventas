...
def _draw_barcode_label(
    ...
    *, max_width_mm=50.0, margin_mm=4.0, name_px=18, code_px=12, vgap_mm=1.5,
    # si ya tienes una forma de obtener la imagen del código, pásala aquí:
    barcode_img: QImage | None = None,
):
    """
    Dibuja una etiqueta: [barcode centrado] + [código (6px)] + [nombre (12px, 1 línea elidada)].
    ...
    # --- BARCODE ---
    # Usa tu rutina actual para generar/dibujar el código de barras.
    # Si ya lo obtienes como QImage/QPixmap, escálalo a 'w' manteniendo aspecto.
    # EJEMPLO seguro (si te llega QImage):
    if isinstance(barcode_img, QImage) and not barcode_img.isNull():
        ...
        # fallback mínimo: rectángulo placeholder
        ph = int(12 * pxmm)  # ~12mm de alto
        painter.fillRect(QRect(x, y, w, ph), Qt.black)
        painter.fillRect(QRect(x+2, y+2, w-4, ph-4), Qt.white)
        y += ph + vgap

    # --- TEXTO: código (6 px) ---
    f_code = QFont("Arial"); f_code.setPixelSize(code_px)
    ...
    # --- TEXTO: nombre (12 px, UNA SOLA línea, elidado) ---
    f_name = QFont("Arial"); f_name.setPixelSize(name_px)
...
class ProductosDialog(QDialog):
    ...
    def dlg_imprimir_codigos(self):
        ...
        # 2) Preparar (código, nombre) desde columnas 2 y 3
        ...
        # 3) Configurar impresora (usa impresora por defecto guardada, si existe)
        cfg = load_config()
        default_name = (cfg.get('printers', {}) or {}).get('barcode_printer')

        printer = QPrinter(QPrinter.HighResolution)
        printer.setResolution(203)  # 203 o 300 si tu impresora soporta
        try:
        ...
        # 4) Tamaño en cm
        w_cm, ok1 = QInputDialog.getDouble(self, 'Imprimir códigos', 'Ancho (cm):', 8.0, 1.0, 50.0, 1)
        if not ok1:
            return
        h_cm, ok2 = QInputDialog.getDouble(self, 'Imprimir códigos', 'Alto (cm):', 20.0, 1.0, 100.0, 1)
        if not ok2:
            return

        printer.setPaperSize(QSizeF(QSize(int(w_cm*10), int(h_cm*10))), QPrinter.Millimeter)
        ...
        # Helper local mm→px (usa resolución actual de impresora si está disponible)
        def mm_to_px(mm: float) -> int:
            dpi = printer.resolution() or  (printer.logicalDpiX() if hasattr(printer, "logicalDpiX") else 96)
        ...
        try:
            ...
            margin_mm = 5.0                   # margen en milímetros
            y = mm_to_px(margin_mm)           # coordenada inicial en px

            for code, name in items:
                # Dibujo de UNA etiqueta (save/restore balanceado)
                try:
                    painter.save()
                    # Trasladar para que el helper siempre arranque desde 'margin'
                    painter.translate(0, y - mm_to_px(margin_mm))
                    img = _barcode_qimage_from_code128(code)
                    # Llama al helper NUEVO con margin_mm (no 'margin')
                    y_end = _draw_barcode_label(
                        painter, width, code, name_text=name,
                        max_width_mm=50.0, margin_mm=4.0, 
                        name_px=14, code_px=8,
                        barcode_img=img ,
                        )
                finally:
                    painter.restore()
                ...
                if y_end + mm_to_px(8.0) > page.height():
                    printer.newPage()
